        ...
        <div className="bg-white border border-zinc-200 rounded-xl shadow-sm overflow-hidden h-full flex flex-col">
            <div className="p-4 border-b border-zinc-100 bg-zinc-50/50 flex flex-col gap-4">
                <div className="flex justify-between items-center">
                    ...
                    <span className="text-[10px] font-mono text-zinc-400 flex items-center gap-1"><Clock size={10}/> Real-Time</span>
                </div>
                
                {/* Solvency Dashboard */}
                <div className={`p-3 rounded-lg border flex justify-between items-center ${isSolvencyRisk ? 'bg-rose-50 border-rose-100' : 'bg-emerald-50 border-emerald-100'}`}>
                    <div>
                         <p className="text-[9px] font-bold uppercase tracking-wider text-zinc-500">Days Cash on Hand</p>
                         <p className={`text-xl font-mono font-bold ${isSolvencyRisk ? 'text-rose-700' : 'text-emerald-700'}`}>
                            {daysOfCash} Days
                         </p>
                    </div>
                    {isSolvencyRisk ? <AlertOctagon size={24} className="text-rose-400" /> : <CheckCircle2 size={24} className="text-emerald-400" />}
                </div>

                <div className="h-16 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                         <AreaChart data={forecastData}>
                            <Tooltip contentStyle={{fontSize: '10px'}} formatter={(val: number) => formatCurrency(val)}/>
                            <Area type="monotone" dataKey="balance" stroke={isSolvencyRisk ? "#e11d48" : "#059669"} fill={isSolvencyRisk ? "#ffe4e6" : "#d1fae5"} strokeWidth={2} />
                         </AreaChart>
                    </ResponsiveContainer>
                    <p className="text-[9px] text-center text-zinc-400 mt-1">10-Day Projection (Linear Burn)</p>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                <table className="w-full text-left">
                    <thead className="bg-white sticky top-0 border-b border-zinc-100">
                        <tr>
                            <th className="p-2 text-[9px] font-bold text-zinc-400 uppercase bg-zinc-50/90">Activity</th>
                            <th className="p-2 text-[9px] font-bold text-zinc-400 uppercase text-right bg-zinc-50/90">Net Impact</th>
                        </tr>
                    </thead>
