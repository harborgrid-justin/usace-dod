...
const CDORateManager: React.FC<Props> = ({ pools, onUpdateRate }) => {
    ...
    return (
        <div className={`flex-1 bg-white border border-zinc-200 rounded-3xl shadow-sm overflow-hidden flex flex-col h-full animate-in fade-in transition-opacity ${isPending ? 'opacity-70' : 'opacity-100'}`}>
            <div className="p-8 border-b border-zinc-100 bg-zinc-50/50 flex flex-col lg:flex-row justify-between items-center gap-6">
                ...
                <div className="flex gap-3 w-full lg:w-auto">
                    <div className="px-4 py-2 bg-emerald-50 text-emerald-800 rounded-xl border border-emerald-100 text-[10px] font-bold uppercase flex items-center gap-2 shadow-sm">
                        <ShieldCheck size={14}/> Rates Locked
                    </div>
                    <button className="flex-1 lg:flex-none px-6 py-2 bg-white border border-zinc-200 text-zinc-700 rounded-xl text-xs font-bold uppercase hover:bg-zinc-50 transition-all">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    {pools.map(pool => (
                        <div key={pool.id} className="bg-white border border-zinc-200 rounded-3xl p-6 hover:shadow-xl hover:border-rose-200 transition-all group flex flex-col h-[280px]">
                            <div className="flex justify-between items-start mb-8">
                                <div className="flex items-center gap-4">
                                    <div className="p-3 bg-zinc-900 text-white rounded-2xl shadow-xl"><Landmark size={20}/></div>
                                    <div>
                            ...
                            <div className="mb-6 p-5 bg-zinc-50 rounded-2xl border border-zinc-100 flex flex-col items-center justify-center text-center">
                                    ...
                                    <div className="flex gap-2 animate-in slide-in-from-bottom-2">
                                        <div className="relative flex-1">
                                             ...
                                             <input 
                                                ...
                                                className="w-full bg-white border border-zinc-900 rounded-xl py-2 pl-7 pr-2 text-sm font-mono font-bold outline-none shadow-lg"
                                            />
                                        </div>
                                        <button onClick={() => handleConfirm(pool.id)} className="p-2 bg-zinc-900 text-white rounded-xl hover:bg-zinc-800 shadow-lg"><Check size={18}/></button>
                                        <button onClick={() => setEditingPoolId(null)} className="p-2 bg-zinc-100 text-zinc-500 rounded-xl hover:bg-zinc-200"><X size={18}/></button>
                                    </div>
                                ) : (
                                    <button 
                                        onClick={() => { setEditingPoolId(pool.id); setNewRate(pool.currentRate); }} 
                                        className="w-full py-3 border border-zinc-200 rounded-2xl text-[10px] font-bold uppercase tracking-widest text-zinc-500 hover:text-zinc-900 hover:border-zinc-900 hover:bg-zinc-50 transition-all flex items-center justify-center gap-2"
                                    >
