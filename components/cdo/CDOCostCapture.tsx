...
const CDOCostCapture: React.FC<Props> = ({ transactions, onAddTransaction, onSelectProject }) => {
    ...
    return (
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 h-full min-h-0 animate-in fade-in overflow-hidden">
            {/* Form Column */}
            <div className="lg:col-span-4 bg-white border border-zinc-200 rounded-md p-8 overflow-y-auto custom-scrollbar shadow-sm">
                <div className="flex items-center gap-4 mb-8 pb-6 border-b border-zinc-100">
                    <div className="p-3 bg-zinc-900 text-white rounded-sm shadow-xl"><Calculator size={24} /></div>
                    <div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-4 p-5 bg-zinc-50 rounded-sm border border-zinc-100">
                        <div>
                            <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-1 mb-2 block">Target Functional Pool</label>
                            <select 
                                className="w-full bg-white border border-zinc-200 rounded-sm p-3 text-sm font-bold focus:border-rose-400 outline-none transition-all appearance-none" 
                                value={form.function} 
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {['Labor', 'Non-Labor', 'Accrual'].map(type => (
                                <button 
                                    ...
                                    className={`py-2 rounded-sm text-[9px] font-bold uppercase border transition-all ${
                    ...
                    {form.type === 'Labor' && (
                        <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2">
                            <div>
                                ...
                                <input type="text" className="w-full mt-1 border border-zinc-200 rounded-sm p-2.5 text-xs font-mono" value={form.employeeId || ''} onChange={e => setForm({...form, employeeId: e.target.value})} placeholder="E-1234"/>
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-zinc-500 uppercase ml-1">Hours</label>
                                <input type="number" step="0.25" className="w-full mt-1 border border-zinc-200 rounded-sm p-2.5 text-xs font-mono" value={form.hours || ''} onChange={e => setForm({...form, hours: Number(e.target.value)})}/>
                            </div>
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        <div className="relative group">
                            ...
                            <input 
                                type="number" 
                                className="w-full border border-zinc-200 rounded-sm pl-10 pr-4 py-4 text-2xl font-mono font-bold text-zinc-900 focus:outline-none focus:border-rose-400 transition-all group-hover:bg-zinc-50/50" 
                                value={form.amount || ''} 
                    ...
                    <div>
                        ...
                        <textarea 
                            className="w-full border border-zinc-200 rounded-sm p-4 text-sm resize-none focus:outline-none focus:border-rose-400 h-32 leading-relaxed" 
                    ...
                    {error && (
                        <div className="p-4 bg-rose-50 border border-rose-100 rounded-sm flex gap-3 text-rose-700 text-xs font-bold animate-in zoom-in">
                            <AlertTriangle size={18} className="shrink-0" />
                            <span>{error}</span>
                        </div>
                    )}

                    <button type="submit" disabled={isPending} className="w-full py-4 bg-zinc-900 text-white rounded-sm text-xs font-bold uppercase tracking-widest hover:bg-zinc-800 transition-all shadow-xl flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50">
            ...
            {/* List Column */}
            <div className="lg:col-span-8 bg-white border border-zinc-200 rounded-md shadow-sm overflow-hidden flex flex-col">
